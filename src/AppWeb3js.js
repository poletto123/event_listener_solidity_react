...
function App() {
  ...
  async function callContract() {
    ...
    //
    let contract = new web3.eth.Contract(abi, addressFrom);
    contract.events.TokensSent().on('data', (event) => {
      setEvents(JSON.stringify(event));
      console.log(event);
    });
    // Filtering for any transactions in the past to specific address
    //
    // contract.getPastEvents("TokensSent", { filter: { to: "0x7BF1C94BB593296a634C1F32b8b6E5B5A58EB8A7" }, fromBlock: 0 })
    //   .then((event) => {
    //     setEvents(JSON.stringify(event));
    //     console.log(event);
    //   });
